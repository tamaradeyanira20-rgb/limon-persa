...
const Timer24 = ({ lastClaimed, onClaim, loading }) => {
  ...
  return (
    <div style={{ textAlign: "center" }}>
      {isSunday && (
        <div style={{ background: "rgba(251,191,36,.08)", border: "1px solid rgba(251,191,36,.2)", borderRadius: 10, padding: "8px 12px", marginBottom: 10 }}>
          <p style={{ color: "var(--gold)", fontSize: 12, fontWeight: 600 }}>🌙 Domingo — Sin rendimiento hoy</p>
          <p style={{ color: "var(--muted)", fontSize: 11 }}>Vuelves a cobrar el lunes a la misma hora</p>
        </div>
      )}
    ...
    useEffect(() => {
      const loadDetails = async () => {
        ...
        for (const r of items) {
          const purchases = await sb(`purchases?user_id=eq.${r.id}&select=*,products(name,price)`).catch(() => []);
          const totalInvested = (purchases || []).reduce((s, p) => s + (p.products ? Number(p.products.price) : 0), 0);
          const commission = totalInvested * (pct / 100);
          map[r.id] = { purchases: purchases || [], totalInvested, commission, hasActive: (purchases || []).some(p => p.is_active) };
        }
        setDetails(map);
      };
        ...
        {items.length === 0
          ...
          : <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {items.map((r, i) => {
                ...
                const hasInvested = d && d.totalInvested > 0;
                return (
                  <div key={i} className="card" style={{ padding: "12px 14px", borderLeft: `3px solid ${hasInvested ? color : "var(--muted)"}` }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                      <div>
                        <p style={{ fontWeight: 600, fontSize: 13 }}>{r.phone}</p>
                        <p style={{ color: "var(--muted)", fontSize: 11 }}>Se unió: {new Date(r.created_at).toLocaleDateString("es-MX")}</p>
                      </div>
                      <span style={{ fontSize: 11, padding: "3px 8px", borderRadius: 10, background: hasInvested ? color + "22" : "rgba(100,100,100,.15)", color: hasInvested ? color : "var(--muted)", fontWeight: 700 }}>
                        {hasInvested ? "✓ Activo" : "Sin compra"}
                      </span>
                    </div>
                    {d && hasInvested && (
                      <div style={{ marginTop: 10, background: "var(--bg)", borderRadius: 10, padding: "10px 12px" }}>
                        {d.purchases.filter(p => p.products).map((p, j) => (
                          <div key={j} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: j < d.purchases.length - 1 ? 6 : 0 }}>
                            <div>
                              <p style={{ fontSize: 12, fontWeight: 600 }}>{p.products.name}</p>
                              <p style={{ color: "var(--muted)", fontSize: 11 }}>Inversión: {fmt(p.products.price)}</p>
                            </div>
                            <div style={{ textAlign: "right" }}>
                              <p style={{ color, fontWeight: 700, fontSize: 13 }}>+{fmt(Number(p.products.price) * pct / 100)}</p>
                              <p style={{ color: "var(--muted)", fontSize: 10 }}>tu comisión</p>
                            </div>
                          </div>
                        ))}
                        <div style={{ borderTop: "1px solid var(--border)", marginTop: 8, paddingTop: 8, display: "flex", justifyContent: "space-between" }}>
                          <span style={{ color: "var(--muted)", fontSize: 12 }}>Total comisión generada</span>
                          <span style={{ color, fontWeight: 800, fontSize: 14 }}>+{fmt(d.commission)}</span>
                        </div>
                      </div>
                    )}
                    {d && !hasInvested && (
                      <p style={{ color: "var(--muted)", fontSize: 12, marginTop: 8 }}>⏳ Aún no ha comprado ningún paquete</p>
                    )}
                  </div>
                );
              })}
...
const VipSection = ({ user, onRefresh }) => {
  ...
  const nextVip = claimedLevel < 5
    ? vipLevels.find(v => v.level <= 5 && qualifiedCount < v.required_refs)
    : vipLevels.find(v => v.level === claimedLevel + 1 && qualifiedCount < v.required_refs);
  const unclaimedLevels = vipLevels.filter(v => {
    if (v.level <= claimedLevel) return false;
    if (claimedLevel < 5) return v.level <= 5 && qualifiedCount >= v.required_refs;
    return v.level === claimedLevel + 1 && qualifiedCount >= v.required_refs;
  });
